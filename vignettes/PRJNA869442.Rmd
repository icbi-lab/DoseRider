...
gmt <- loadCPDB("Symbol")
gmt <- filter_gmt_by_size(gmt = gmt, minGenesetSize = 5, maxGenesetSize = 200)  
```
...
```{r}
res <- DoseRider(bpaf, gmt = gmt, dose_col = "dose", sample_col = "description", omic = "proteomics", minGSsize = 5, maxGSsize = 300)
...
```{r}
plot_smooth(res,"MAPK signaling pathway - Homo sapiens (human)")
res$`Pathways of neurodegeneration - multiple diseases - Homo sapiens (human)`$Smooth_Predictions
```
