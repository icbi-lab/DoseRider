...
gmt <- loadCPDB("Symbol")
gmt <- filter_gmt_by_size(gmt = gmt, minGenesetSize = 10, maxGenesetSize = 20)  
```
...
```{r}
res <- DoseRiderParallel(bpaf, gmt = gmt, omic = "proteomics", minGSsize = 10, maxGSsize = 200, num_cores = 30)
resDf <- as.data.frame(res)
table(unlist(resDf["P_Value_Cubic"]) < 0.05)
...
```{r}
plot_smooth(res,"Tamoxifen Action Pathway", center_values = T)
```
