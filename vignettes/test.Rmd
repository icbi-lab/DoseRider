...
lSign <-  (res_df$best_model == "non_linear_mixed" & res_df$adjusted_non_linear_mixed_p_value < 0.001) |
          (res_df$best_model == "non_linear_fixed" & res_df$adjusted_non_linear_fixed_p_value < 0.001) |
          (res_df$best_model == "linear" & res_df$adjusted_linear_p_value < 0.001) 

#save(res, file = "../../doseRider_save/res_bpa_20_1000.rda")
filter_DoseRider(dose_rider_results)
```
...
```{r}
top <- 15
save_path <- "/home/monfortl/plots/Example_results/MSIGDB/"
...
for (geneset in res_df$Geneset[lSign]) {
  center_values = ifelse(dose_rider_results[[geneset]]$best_model == "linear", F , ifelse(
    dose_rider_results[[geneset]]$best_model == "non_linear_fixed",F,T
  ))
  p9 <- plot_pathway_response(dose_rider_results, gene_set_name = geneset, center_values = center_values) + theme(plot.margin = margin(1,1,1.5,1.2, "cm")) + ggtitle(stringr::str_wrap(geneset,10))

  plotFile9 <- paste0(save_path,"/",stringr::str_replace_all(geneset,"/","_"),".jpeg")
  ggsave(plotFile9, plot = p9, width = 150, height = 150, units = "mm", dpi = 600)
}

p4 <- plot_gene_random_effect_relationship(dose_rider_results, "DUTERTRE_ESTRADIOL_RESPONSE_6HR_DN")
...
p6 <- create_gene_heatmap(dose_rider_results,dose_col = "Dose", gene_set_name = "DUTERTRE_ESTRADIOL_RESPONSE_6HR_DN")
