...
compound_doses <- list("BPAF" = c(0.0005, 0.001, 0.01, 0.1, 0.5, 1, 5))
doses <- c(0, compound_doses["BPAF"][[1]])

# Filter data for BPAF
bpaf_data <- PRJNA869442[
  (colData(PRJNA869442)$Chemical %in% c("Cells, no treatment", "BPAF")) &
...
gmt <- filter_gmt_by_size(read_gmt(gmt_perturbation_path), 20, 1000)
```
...
p1 <- dose_response_heatmap(dose_rider_results, dose_col = "Dose", top = top, order_column = "best_model_pvalue", decreasing = FALSE)
